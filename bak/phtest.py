...
def train(expnum, HD_Model, syn_data, step_flag, device, celeba_data=None, read_first=None,
    ...
    ph_val_dl  = DataLoader(val_dataset, batch_size=8, shuffle=False)
    ...
    with torch.no_grad():
        ...
        for bix, batch in enumerate(ph_val_dl):
            ...
            mask = mask.to(device)
            gt_n = gt_n.to(device)
            ph_face = ph_face * mask
            pre_norm = F.normalize(pre_norm)

            pA_1, pS_1, pN_1, pL_1, pA_2, pS_2, pN_2, pL_2 = HD_Model(ph_face, ph_face) #[-1, 1]
            pN_1 = F.normalize(pN_1) * mask
            pN_2 = F.normalize(pN_2) * mask

            ph_fine_normal = normal_DPR2SFS(F.normalize(pN_1) * mask)
            ...
            # print(bix)
            # s_immg1 = torch.cat([ph_face*mask, get_normal_in_range(gt_n)*mask, get_normal_in_range(ph_fine_normal)*mask], dim=0)
            # save_image(s_immg1, 'img1.png', nrow=b, normalize=False)


        print("===> Avg. mean: {:.4f}, std:{:.4f}, n20:{:.4f}, n25:{:.4f}, n30:{:.4f}".format(sum_mean / testflag, sum_std / testflag, sum_n20 / testflag, sum_n25 / testflag, sum_n30 / testflag))
        with open('mean_std.txt', 'a+') as f:
            details = str(10086) + '\t' + str(sum_mean / testflag) + '\t' + str(sum_std /testflag) + '\t' + str(sum_n20 /testflag) + '\t' + str(sum_n25 /testflag) + '\t' + str(sum_n30 /testflag) + '\n'
            f.write(details)     
