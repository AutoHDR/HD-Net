...
def train(HD_Model, data_path, step_flag, device, batch_size = 8, num_epochs = 20, log_path = './results/HD-Net/', lr = 0.001, wt_decay=0.005):
    ...
    train_dataset, _ = get_dataset_sy(read_from_csv=train_csv, validation_split=0)
    test_dataset, _ = get_dataset_sy(read_from_csv=test_csv, validation_split=0)
    ...
    print('celeba dataset: Train data: ', len(train_dl), ' Test data: ', len(test_dl))

    out_syn_images_dir = log_path + '/' + step_flag + '/'
    model_checkpoint_dir = out_syn_images_dir  + '/' + 'checkpoints/'
    ...
    testpath = out_syn_images_dir + '/'+ 'test/'

    tar_path = out_syn_images_dir + '/' + '_code.tar.gz'
    make_targz(tar_path, '.')
    writer_path = out_syn_images_dir + '/'
    ...
    DIS_F = Discriminator(input_shape).to(device)
    optimizer_DIS_F = torch.optim.Adam(DIS_F.parameters(), lr=lr, weight_decay=wt_decay)

    D_model_path = '/media/hdr/oo/result/Public/stage_1/HD_Net32_DeL/checkpoints/20__DF.pkl'
    DIS_F.load_state_dict(torch.load(D_model_path))
    ...
    for epoch in range(1, num_epochs+1):
        for bix, data in enumerate(train_dl):
            ...
            valid = torch.ones((b, *DIS_F.output_shape), requires_grad=False).to(device)
            fake = torch.zeros((b, *DIS_F.output_shape), requires_grad=False).to(device)
            ...
            E_A = lambda_A * (HD_L1loss(pA_1, pA_2))  
            E_R_1 = lambda_R * (HD_L1loss(rec_F_1, face_1) + HD_L1loss(rec_F_2, face_2)) 
            ...
            E_N = lambda_N *(HD_L2loss(pN_1, normal) + HD_L2loss(pN_2, normal)) #+ HD_L2loss(pN_2, pN_1)

            l_1 = get_L_DPR_B(pS_1, normal).detach() # stage 1
            l_2 = get_L_DPR_B(pS_2, normal).detach()
            # l_1 = get_L_DPR_B(pS_1, pN_1).detach() # stage 2
            # l_2 = get_L_DPR_B(pS_2, pN_2).detach()

            E_L = lambda_L *(HD_L1loss(pL_1, l_1) + HD_L1loss(pL_2, l_2))

            optimizer_DIS_F.zero_grad()
            loss_real_1 = criterion_GAN(DIS_F((rec_S_1*pA_1*mask).detach()), fake)
            loss_fake_3 = criterion_GAN(DIS_F(face_1), valid)
            D_loss = loss_real_1 + loss_fake_3
            D_loss.backward()
            optimizer_DIS_F.step()


            D_R = lambda_G * (criterion_GAN(DIS_F(rec_S_1*pA_1*mask), valid)  + criterion_GAN(DIS_F(rec_S_2*pA_2*mask), valid))

            total_loss = E_R_1 + E_kind + E_S_NL + E_N + E_A + E_L + D_R
            total_loss.backward()
            optimizer.step()
            ...
            if bix % 1==0:
                file_name = out_syn_images_dir + '/train/' + str(epoch)
        ...
        torch.save(DIS_F.state_dict(), model_checkpoint_dir + str(epoch) + '_' + '_DF.pkl')

        with torch.no_grad():
            std, mean, n20, n25, n30, a_mae, a_rmse = test(test_dl, HD_Model, testpath, epoch, device)
